...
if ((int) $captcha !== (int) $valorgenerado) {
  ...
  redireccionConContador("CAPTCHA INCORRECTO", 3, "/practica3.5.2/login.html");
  exit();
}
...
if ($resultado->num_rows > 0) {
  ...
  if (password_verify($contraseña, $fila['password'])) {
    ...
    if ($realiza->execute()) {
      ...
      redireccionConContador("Error al registrar el login del usuario: $usuario. Por favor, inténtalo de nuevo.", 3, "/practica3.5.2/registro.html");
    }
    ...
    require_once 'redirec_tiempo.php';
    redireccionConContador("LOGIN CORRECTO<br><br>$mensaje", 3, "/practica3.5.2/protegida.php");
  } else {
    require_once 'redirec_tiempo.php';
    redireccionConContador("CREDENCIALES INVALIDAS", 3, "/practica3.5.2/login.html");
  }
} else {
  require_once 'redirec_tiempo.php';
  redireccionConContador("CREDENCIALES INVALIDAS", 3, "/practica3.5.2/login.html");
